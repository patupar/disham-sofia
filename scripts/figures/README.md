...
`configs/figures.yaml`. The existing sensor-panel and QGIS LCZ/topography figures are
maintained separately and are not regenerated by these scripts.
...
1. In QGIS, export the existing **district polygon layer**, including both LEZ and other
   districts, to `data/raw/boundaries/sofia_districts_lez.geojson`. Include an integer field
   ...
   as in the existing report map, not a traffic-LEZ boundary. To use another supported
   vector format, change `districts` and, where applicable, `district_layer` in the
...
- The model diagnostic also uses IBM Plex Mono for its metric annotations. It pools
  ...
  axes and density scale. Each panel contains a 0–60 µg/m³ inset that enlarges the dense
  lower-left region while retaining the full-range view. The dashed 1:1 line shows exact
  agreement; no regression line is fitted and no model is retrained. The script verifies
  that the prediction and metrics files describe the same Stage 9b run before drawing
  the figure.
...
- On the two result figures, sensor fill encodes the difference: blue is negative,
  white is zero, and dark pink is positive. All markers have the same size and a thin
  dark outline.
